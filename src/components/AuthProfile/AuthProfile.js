...
function AuthProfile({ name, title, isActive, children, ariaLabel, handleSubmit, gray, blue, link, errorMessageAuth }) {
    ...
    <div className={`auth auth_type_${name}`}>
      <div className="auth__container">
        ...
        <form className={`auth__form-element auth__form-element_type_${name}`} name={`${name}-form`} onSubmit={handleFormSubmit}>
          ...
          <button className={`auth__submit ${buttonClasses}`} type="submit" aria-label={ariaLabel}>
            {ariaLabel}
          </button>
        </form>
